...
export function CartHeaderWidget() {
  ...
  // Слідкуємо за поповненням кошика для запуску кидка
  useEffect(() => {
    if (totalItems > prevItems) {
      setIsAnimating(true);
      const timer = setTimeout(() => setIsAnimating(false), 600); // Час польоту м'яча
  ...
  return (
    <>
      <button 
        ...
        // Твій оригінальний wrapper, лише змінив w-[100px] на min-w-[100px] щоб сума влазила, якщо буде велика
      ...
      >
        ...
        <div className="relative flex h-[30px] w-[45px] shrink-0 items-center justify-center transition-transform duration-500 ease-out group-hover/basket:scale-[1.15]">
          ...
          {/* СТАН 1: Показуємо Суму (якщо є товари і немає анімації) */}
          <span 
            className={cn(
          ...
          {/* СТАН 2: Показуємо М'яч (твоє orange_logo.svg) */}
          <div 
            className={cn(
        ...
        {/* === ПРАВА ЧАСТИНА (ТВІЙ ОРИГІНАЛЬНИЙ БЛОК КОРЗИНИ) === */}
        <div className="absolute right-[2px] top-[4px] z-20 flex h-[44px] w-[44px] shrink-0 items-center justify-center transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover/basket:-translate-y-[4px] group-hover/basket:scale-105">
          <Image
            ...
            className={cn(
              ...
              isAnimating && "scale-110 drop-shadow-[0_0_10px_#EC5800]" // Легкий ефект при попаданні
            )}
          />
          
          {/* Лічильник з'являється тільки коли є товари і кидок завершено */}
          {!isEmpty && !isAnimating && (
      ...
      {/* Траєкторія кидка налаштована спеціально під відстань між твоїми svg */}
