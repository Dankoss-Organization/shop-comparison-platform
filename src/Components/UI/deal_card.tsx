...
export function BaseDealCard({ item, onClick, compact, className = "", size }: { item: DealCardType; onClick?: () => void; compact?: boolean; className?: string; size: any }) {
  ...
  return (
    <article 
      ...
      className={cn("group relative isolate overflow-hidden border border-[#ffffff14] bg-[#342e34] shadow-sm", size.wrapper, clickable ? "cursor-pointer transform-gpu transition duration-300 hover:-translate-y-1" : "", className)}
    >
      <div className={cn("relative overflow-hidden", size.image)}>
        ...
        <div className={compact ? "absolute left-3 right-3 top-3 flex items-start justify-between" : "absolute left-4 right-4 top-4 flex items-start justify-between"}>
          ...
          <div className="relative z-20">
            ...
            >
              {bestOffer ? bestOffer.store_name : "N/A"}
              <svg 
            ...
            {isOffersOpen && item.offers.length > 1 && (
              <div className="absolute left-0 top-[110%] mt-1 flex w-max min-w-[120px] flex-col overflow-hidden rounded-xl border border-[#ffffff1a] bg-[#1a171a]/95 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.5)] animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="px-3 py-2 text-[9px] font-bold uppercase tracking-widest text-[#FFDEBA]/40 border-b border-[#ffffff10]">
                  Available at
                </div>
                <div className="flex flex-col py-1">
                  {[...item.offers]
                    ...
                    .map((offer) => (
                    <div key={offer.store_id} className="flex items-center justify-between gap-4 px-3 py-1.5 hover:bg-[#ffffff0a] transition-colors">
                      <span className="text-[11px] font-bold tracking-wider text-[#FFDEBA]/90">{offer.store_name}</span>
                      <div className="flex flex-col items-end">
                        ...
                        {offer.pricing.discount_percent > 0 && (
                          <span className="text-[9px] text-[#FFDEBA]/40 line-through">${offer.pricing.regular_price.toFixed(2)}</span>
                        )}
                      </div>
                    </div>
                  ))}
        ...
        <div className={compact ? "mt-3 flex items-end justify-between gap-3" : "mt-5 flex items-end justify-between gap-4"}>
          <div>
            <p className={cn(size.price, "font-black text-[#EC5800]")}>
              {bestOffer ? `$${bestOffer.pricing.current_price.toFixed(2)}` : "Unavailable"}
            </p>
          </div>
          <button 
            type="button" 
