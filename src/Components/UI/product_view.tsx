 ...
 * @description Modular UI components for the product detail page.
 */
    ...
    <div className="relative flex-1 overflow-hidden rounded-[1.5rem] border border-glass/10 bg-gradient-to-b from-bg-elevated to-bg-darker p-4 shadow-[0_15px_30px_rgba(0,0,0,0.4)]">
      <div className="absolute left-5 top-5 z-10 flex flex-wrap items-center gap-2">
        ...
        {item.availabilityStatus === "in_stock" && (
          <span className="rounded-full bg-green-500/20 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-green-400">
            In Stock
          </span>
        )}
      </div>
      ...
      <div className="mx-auto mt-10 flex aspect-square w-full max-w-[450px] items-center justify-center overflow-hidden rounded-xl bg-bg-main">
        ...
        <div className="rounded-xl border border-glass/10 bg-bg-elevated p-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-4xl font-black leading-none text-text-primary">
                {item.rating && item.rating !== "0.0" ? item.rating : "-"}
              </p>
            </div>
            <div className="rounded-full bg-brand-orange/10 px-3 py-1.5 text-[11px] font-semibold text-brand-orange">
              Verified reviews
            </div>
          </div>
          <p className="mt-3 text-[13px] leading-relaxed text-text-primary/60">
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-brand-orange">
          {displayCategory}
        </p>
        {item.brand && (
          <>
            <span className="text-text-primary/30">·</span>
            <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-text-primary/50">
              {item.brand}
            </p>
          </>
        )}
      ...
      <h1 className="mt-2 text-3xl font-black leading-tight text-text-main lg:text-4xl">
        {item.title}
      </h1>
...
export function ProductActions({ item, onOfferChange }: { 
  ...
  const parsedQuantity = useMemo(() => {
    ...
    return { isWeight, baseUnit: isWeight ? "g" : "pcs" };
  }, [item.quantity]);
    ...
    <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {sortedOffers.map((offer) => (
              ...
              >
                ...
                {offer.store_city && (
                  <span className="text-[10px] text-text-primary/40">{offer.store_city}</span>
                )}
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-glass/10 bg-bg-darker text-lg font-semibold text-text-primary transition hover:border-brand-orange disabled:cursor-not-allowed disabled:opacity-45"
                >
                  -
                </button>
                ...
                <button
                  ...
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-glass/10 bg-bg-darker text-lg font-semibold text-text-primary transition hover:border-brand-orange"
                >
                  +
                </button>
              </div>
              {activeOffer && (
                <p className="text-[11px] font-medium text-text-primary/40">
      ...
      <button
        ...
        className={cn(
          ...
          added
            ? "bg-green-500 shadow-[0_8px_20px_rgba(34,197,94,0.3)]"
    ...
    <div className="flex flex-col gap-3">
        ...
        <div className="space-y-3 pt-2 text-[13px] leading-relaxed text-text-primary/60">
          <div className="space-y-2">
            {descriptionSections.map((section, index) => (
              <p key={`description-${index}`}>{section}</p>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-2 pt-2">
            {[...item.offers]
              ...
              .map((offer) => (
                ...
                >
                  <div>
                    <p className="text-[13px] font-bold text-text-main">{offer.store_name}</p>
      ...
      <AccordionBlock label="Nutrition" open={expanded.nutrition} onToggle={() => toggle("nutrition")}>
        {hasNutrition ? (
          ...
          <p className="pt-3 text-[13px] text-text-primary/40">
...
function OptionBlock({ label, content }: { label: string; content: ReactNode }) {
  return (
    <div className="h-full rounded-xl border border-glass/10 bg-bg-darker p-4 shadow-sm">
...
function SoftTag({ children }: { children: ReactNode }) {
  return (
    <span className="rounded-full border border-glass/10 bg-bg-elevated px-2.5 py-1 text-[11px] font-semibold text-text-primary">
      {children}
    </span>
  );
}
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-glass/10 bg-bg-darker shadow-sm">
      <button
        type="button"
        onClick={onToggle}
        className="flex w-full items-center justify-between px-5 py-3.5 text-left transition hover:bg-glass/5"
      >
    ...
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {values.map((value) => (
          <span
            key={`${title}-${value}`}
            className="rounded-full border border-brand-orange/20 bg-brand-orange/10 px-2.5 py-1 text-[11px] font-semibold text-text-primary"
          >
            {value}
          </span>
        ))}
    ...
    <div className="rounded-lg border border-glass/10 bg-bg-elevated p-3.5">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <div className="mt-0.5 flex items-center gap-0.5 text-[11px] text-brand-orange">
            {Array.from({ length: 5 }).map((_, index) => (
              <span key={index}>{index < stars ? "\u2605" : "\u2606"}</span>
            ))}
          </div>
        </div>
        <span className="rounded-full bg-brand-orange/10 px-2.5 py-1 text-[10px] font-semibold text-brand-orange">
          Verified
        </span>
      </div>
