 ...
 * Responsive: Organic mobile clustering of User Profile + Basket. Removes tap highlights.
 */
...
export function HeaderContent() {
  ...
  const triggerAuthSuccessHint = useUIStore((state) => state.triggerAuthSuccessHint);
  ...
  useEffect(() => {
    const handleClick = (event: MouseEvent) => {
      ...
      if (
        ...
        !target.closest("#fav-guest-trigger")
      ) {
  ...
  }, [isCatalogOpen, closeCatalog, isProfileOpen, setProfileOpen]);

  const handleGuestFavClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
        ...
        <div className="flex items-center gap-4 sm:gap-8 xl:gap-[60px]">
          <div className="flex items-center">
            <button
              ...
              className="group relative z-[50] flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full bg-glass/5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:bg-brand-orange hover:shadow-[0_8px_20px_rgb(var(--brand-orange)_/_0.4)] active:scale-90 active:translate-y-0 outline-none focus:outline-none [-webkit-tap-highlight-color:transparent] select-none"
              type="button"
            >
          ...
          <div className="hidden lg:flex items-center gap-0">
            ...
            <div className="relative">
              {isAuthenticated ? (
                <Link href="/favorites" className="outline-none group/fav">
                  <ChainIcon className={favoriteAuthHint?.type === "success" ? "shadow-[0_0_18px_rgb(var(--brand-orange)_/_0.35)]" : ""}>
                    <div className="relative flex items-center justify-center transition-opacity hover:opacity-80">
                      <div className="w-[20px] h-[20px] bg-text-primary opacity-70 transition-all duration-300 group-hover:bg-white group-hover:opacity-100 [mask-image:url(/favourites.svg)] [-webkit-mask-image:url(/favourites.svg)] [mask-size:contain] [-webkit-mask-size:contain] [mask-repeat:no-repeat] [-webkit-mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-position:center]" />
                      {isMounted && favoriteCount > 0 && (
                        <span className="absolute -top-[15px] -right-[15px] flex h-[18px] min-w-[18px] items-center justify-center rounded-full border-[2px] border-bg-surface bg-brand-orange px-[4px] text-[9px] font-black leading-none text-white animate-in zoom-in duration-300">
                          {favoriteCount}
                        </span>
                      )}
                    </div>
                ...
                <div
                  id="fav-guest-trigger"
                  className="relative group/fav outline-none cursor-pointer"
                  onClick={handleGuestFavClick}
                >
                  <ChainIcon>
                    <div className="relative flex h-[20px] w-[20px] items-center justify-center">
                      <div className="absolute w-[20px] h-[20px] bg-text-primary opacity-70 transition-all duration-300 group-hover/fav:scale-50 group-hover/fav:opacity-0 [mask-image:url(/favourites.svg)] [-webkit-mask-image:url(/favourites.svg)] [mask-size:contain] [-webkit-mask-size:contain] [mask-repeat:no-repeat] [-webkit-mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-position:center]" />
                      <svg
                  ...
                  <div className={`absolute top-[45px] left-1/2 z-50 w-[160px] -translate-x-1/2 rounded-xl border border-glass/10 bg-bg-surface px-3 py-2 text-center text-[11px] font-semibold text-text-primary shadow-[0_10px_25px_rgba(0,0,0,0.15)] backdrop-blur-md transition-all duration-300 ${
                    isProfileOpen ? "pointer-events-none opacity-0 translate-y-2" : "pointer-events-none opacity-0 translate-y-2 group-hover/fav:translate-y-0 group-hover/fav:opacity-100 group-hover/fav:pointer-events-auto"
                  }`}>
                    Login/Register to sync and view favorites
                  </div>
                </div>
              )}

              <AnimatePresence>
                {favoriteAuthHint?.type === "success" && (
                  <motion.div
            ...
            <div className="relative z-50 flex items-center justify-center h-[42px] w-[42px]">
              ...
              <AnimatePresence>
                {isLangOpen && (
                  ...
                  >
                    <div
                      className="flex w-full flex-col items-center overflow-y-auto pb-[16px] [&::-webkit-scrollbar]:hidden [mask-image:linear-gradient(to_bottom,black_70%,transparent_100%)] [-webkit-mask-image:linear-gradient(to_bottom,black_70%,transparent_100%)]"
          ...
          <div className="flex items-center gap-0 lg:gap-1 lg:pl-0 relative z-50">
            <div className="relative flex items-center justify-center">
              <div
                ...
                className="group flex items-center justify-center w-[42px] h-[42px] hover:opacity-80 transition-opacity outline-none focus:outline-none [-webkit-tap-highlight-color:transparent] select-none border-none ring-0 bg-transparent"
              >
                <ChainIcon>
                  <div className="w-[20px] h-[20px] bg-text-primary opacity-70 transition-all group-hover:bg-white group-hover:opacity-100 [mask-image:url(/user.svg)] [-webkit-mask-image:url(/user.svg)] [mask-size:contain] [-webkit-mask-size:contain] [mask-repeat:no-repeat] [-webkit-mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-position:center]" />
                </ChainIcon>
              </div>
...
function AnimatedCatalogIcon({ isOpen }: { isOpen: boolean }) {
  return (
    <>
      <svg
        viewBox="0 0 70 57" fill="none" xmlns="http://www.w3.org/2000/svg"
        className={`absolute w-[24px] h-[24px] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? "opacity-0 scale-50 rotate-90" : "opacity-100 scale-100 rotate-0 group-hover:opacity-0 group-hover:scale-50 group-hover:rotate-90"}`}
      >
      ...
      <svg
        viewBox="0 0 83 93" fill="none" xmlns="http://www.w3.org/2000/svg"
        className={`absolute w-[26px] h-[26px] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? "opacity-100 scale-100 rotate-0" : "opacity-0 scale-50 -rotate-90 group-hover:opacity-100 group-hover:scale-100 group-hover:rotate-0"}`}
      >
...
function BrandLogo() {
  return (
    <Link href="/" className="relative flex min-w-[50px] sm:min-w-[220px] select-none items-center justify-center py-2 cursor-pointer group/logo">
      <div
        className="absolute inset-0 z-0 scale-[1.2] bg-bg-elevated shadow-sm dark:shadow-none transition-all duration-700 ease-out opacity-80 dark:opacity-60 group-hover/logo:scale-[1.3] group-hover/logo:opacity-100 dark:group-hover/logo:opacity-100 hidden sm:block [mask-image:url(/dankoss_logo_bkg.svg)] [-webkit-mask-image:url(/dankoss_logo_bkg.svg)] [mask-size:contain] [-webkit-mask-size:contain] [mask-repeat:no-repeat] [-webkit-mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-position:center]"
      />
      <span className="relative z-10 flex items-center text-[28px] sm:text-[35px] font-bold tracking-[0.1em] text-text-primary transition-colors duration-500 group-hover/logo:text-text-main group-hover/logo:drop-shadow-[0_0_10px_rgb(var(--brand-orange)_/_0.3)]">
        <span className="hidden sm:block">DANK</span>
        <span className="relative mx-[2px] flex h-[28px] w-[28px] sm:h-[35px] sm:w-[35px] items-center justify-center">
          ...
          <span className="opacity-0 absolute hidden sm:block">O</span>
          <svg viewBox="0 0 61 72" fill="none" xmlns="http://www.w3.org/2000/svg" className="absolute w-[36px] h-[36px] sm:w-[45px] sm:h-[45px] top-[-5px] transition-transform duration-500 group-hover/logo:scale-105">
        ...
        <span className="hidden sm:block">SS</span>
      </span>
    </Link>
  );
}