 ...
 * and integrates global UI state for seamless authentication hints and dropdowns.
 */
 ...
 * Manages the state of the language dropdown, handles outside clicks to close
 * floating elements, and renders navigation items including the logo, search bar, 
 * action icons, and authentication hints.
 */
export function HeaderContent() {
  ...
  /**
   * @brief Handles guest interactions with the favorites icon.
   * Opens the profile dropdown and visually bounces the Sign In button.
   */
  ...
  /**
   * @brief Triggers authentication success state and visual hints.
   */
            ...
            <div className="relative">
              {isAuthenticated ? (
                ...
                >
                  <ChainIcon>
                    <div className="relative flex h-[20px] w-[20px] items-center justify-center">
                      <Image 
                        ...
                        width={20} 
                        height={20} 
                        className={`absolute transition-all duration-300 group-hover/fav:opacity-0 group-hover/fav:scale-50 ${favoriteCount > 0 ? "opacity-100 text-[#EC5800]" : "opacity-80"}`} 
                        style={favoriteCount > 0 ? { filter: 'drop-shadow(0 0 8px rgba(236,88,0,0.5))' } : {}}
                      />
                      <svg
                        viewBox="0 0 24 24"
                        ...
                        className="absolute inset-0 m-auto h-[20px] w-[20px] opacity-0 transition-opacity duration-300 group-hover/fav:opacity-100"
                      >
                      ...
                      {isMounted && favoriteCount > 0 && (
                        <span 
                          key={favoriteCount}
                          className="absolute -top-[15px] -right-[15px] flex h-[18px] min-w-[18px] items-center justify-center rounded-full border-[2px] border-[#2B262C] bg-[#EC5800] px-[4px] text-[9px] font-black leading-none text-white transition-opacity duration-300 group-hover/fav:opacity-0"
                        >
                          {favoriteCount}
                        </span>
                      )}
                  ...
                  <div className={`absolute top-[45px] left-1/2 z-50 w-[160px] -translate-x-1/2 rounded-xl border border-[#EC5800]/30 bg-[rgba(26,24,28,0.95)] px-3 py-2 text-center text-[11px] font-semibold text-[#FFDEBA] shadow-[0_10px_25px_rgba(0,0,0,0.5)] backdrop-blur-md transition-all duration-300 ${
                    isProfileOpen
              ...
              <AnimatePresence>
                {favoriteAuthHint?.type === "success" && (
                  <motion.div
                    ...
                    className="absolute left-1/2 top-[46px] z-[80] w-[228px] rounded-2xl border border-[#EC580033] bg-[#1A181CEB] px-4 py-3 text-center shadow-[0_18px_40px_rgba(0,0,0,0.45)] backdrop-blur-xl"
                  >
                    <p className="text-[12px] font-bold tracking-[0.04em] text-[#B7F7C1]">
                      {favoriteAuthHint.title}
                    </p>
                    <p className="mt-1 text-[11px] leading-relaxed text-[#FFDEBAA6]">
...
function AnimatedCatalogIcon({ isOpen }: { isOpen: boolean }) {
  return (
    <>
      ...
      <svg viewBox="0 0 83 93" fill="none" xmlns="http://www.w3.org/2000/svg" 
