    ...
    <div className="relative flex flex-col gap-8 sm:gap-10 w-full pb-10 z-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 sm:gap-6 border-b border-text-main/5 dark:border-white/5 pb-5 md:pb-6">
        <div className="flex flex-col gap-1.5 sm:gap-2">
          ...
          <p className="text-[14px] sm:text-[15px] text-text-muted dark:text-text-primary/50">Stay updated on price drops, targets, and personal promos.</p>
        </div>
            ...
            alerts.map((alert) => (
              ...
              >
                ...
                <div className="flex items-start gap-3 sm:gap-5 p-4 pl-5 sm:p-6 sm:pl-8">
                  <div className={`flex h-10 w-10 sm:h-14 sm:w-14 shrink-0 items-center justify-center rounded-xl sm:rounded-2xl text-[18px] sm:text-[24px] shadow-sm border transition-all ${
                    alert.unread 
                  ...
                  <div className="flex flex-col flex-1 gap-1 sm:gap-1.5 pt-0 sm:pt-1">
                    <div className="flex justify-between items-start gap-2 sm:gap-4">
                      <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                        <span className={`text-[15px] sm:text-[17px] font-bold font-serif leading-tight ${alert.unread ? 'text-text-main dark:text-white' : 'text-text-main dark:text-text-primary'}`}>
                          {alert.title}
                        </span>
                    ...
                    <p className={`text-[13px] sm:text-[14px] leading-relaxed pr-2 sm:pr-8 ${alert.unread ? 'text-text-main dark:text-text-primary' : 'text-text-muted'}`}>
                      {alert.message}
                    </p>
